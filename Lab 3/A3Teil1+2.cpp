...
bool operator!=(circle& cir1, circle& cir2);
inline float area(circle& cir);
circle operator*(float num, circle& cir);
bool operator<(circle& cir1, circle& cir2);
bool operator<=(circle& cir1, circle& cir2);
...
ostream& operator<<(ostream& out, point2D& pnt){
    return out<<"(" <<pnt.x_coordinate <<"," <<pnt.y_coordinate <<")";
}
...
ostream& operator<<(ostream& out, circle& cir){
    return out<<"center=" <<cir.center <<", radius=" <<cir.radius <<", area=" <<area(cir) <<endl;
}
...
inline float area(circle& cir){
    float area = 3.14159 * cir.radius * cir.radius;
    return area;
}
...
circle operator*(float num, circle& cir){
    circle new_cir;
    new_cir.center = cir.center;
    if(num<0){
        cerr<<"error in operator*: factor with negative value, multiplication with sign changed value instead" <<endl;
        num*=-1;
    }
    new_cir.radius = cir.radius * num;
    return new_cir;
...
bool operator<(circle& cir1, circle& cir2){
    if(area(cir1) < area(cir2)) return true;
    else return false;
}
...
int main(void)
{
    ...
    //new
    float mult;
    cout<<"input factor to multiply with the radius of each circle: ";
    cin>> mult;
    for(int i=0, j=6; i<6;i++, j++){
        c[j] = operator*(mult, c[i]);
    }
    ...
    for(int i=0;i<12;i++){
        cout<<"c[" <<i <<"] < c[0] is " <<boolalpha <<operator<(c[i], c[0])<<endl;
        cout<<"c[" <<i <<"] <= c[0] is " <<boolalpha <<operator<=(c[i], c[0])<<endl;
    }
